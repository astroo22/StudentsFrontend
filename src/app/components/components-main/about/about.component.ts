...
export class AboutComponent implements AfterViewInit {
  ...
  aboutProjectTextpt2 = `This project is imbued with advanced features expected in professional business environments, demonstrating a seamless CI/CD pipeline utilizing AWS CodePipeline, CodeBuild, and CodeDeploy.
   It employs Angular for the frontend and Go for the backend, maintaining a RESTful approach for handlers to guarantee standardized interactions throughout the application. The architecture involves hosting the frontend on an S3 bucket and the backend on an EC2 server, with both development and production databases under Amazon RDS, utilizing PostgreSQL.
  Security is paramount; hence, stringent authentication is required on both frontend and backend to make alterations, coupled with CloudFront and NGINX routing level security to thwart malicious requests.
   All components are encapsulated within a secure HTTPS protocol, ensuring data integrity and confidentiality. Please note, as I am in pursuit of employment opportunities, the resources are currently operating on AWS's free tier, restricting school generation to a maximum of 50 students per grade.`;
  
   privacyText = `Rest assured, there is no intention or necessity to retain any data input into the application. Users have full control to delete their accounts and associated data, emphasizing the exemplary and non-commercial nature of this website.
    Any information, including emails, is purely for display purposes, and users are encouraged to use fictitious data as desired. The focus is on showcasing capabilities and ensuring user autonomy and privacy.`;
  ...
  aboutMePlants = `I cultivate a variety of produce, including several types of tomatoes, Hatch green chili, jalapenos, banana peppers, ghost peppers, and potatoes on my apartment balcony.
   Additionally, I nurture indoor plants such as a cayenne pepper plant and an assortment of vines and succulents. Whether it’s a Japanese Emperor tree or the unexpected transformation of cabbage into a peculiar lettuce stick, each plant has its unique story.`;
